...
typedef enum { DIRECT_ON, DIRECT_OFF } direct_mode_t;
...
static int parse_direct_mode(const char* value, direct_mode_t* mode) {
  if (strcmp(value, "on") == 0) {
    ...
    *mode = DIRECT_OFF;
    ...
    (void)fprintf(
        stderr, "Invalid direct mode: %s (must be 'on' or 'off')\n", value
    );
    return -1;
  }
  return 0;
...
static int parse_arguments(
  ...
  if (params->direct == DIRECT_ON && params->block_size % page_size != 0) {
    (void)fprintf(
...
static int perform_io_operation(
  ...
  ssize_t result = 0;
  if (params->rw_mode == MODE_READ) {
    result = read(file_descriptor, buffer, params->block_size);
  } else {
    result = write(file_descriptor, buffer, params->block_size);
  }
...
void* thread_work(void* arg) {
  ...
  io_params_t* params = (io_params_t*)arg;
  ...
  int file_descriptor = open(params->filename, flags, FILE_MODE);
...
int main(int argc, char* argv[]) {
  ...
  if (parse_arguments(argc, argv, &params, page_size) != 0) {
    (void)fprintf(
        ...
        "<count> "
        "--file <path> [--range <start-end>] [--direct <on|off>] [--type "
        "<sequence|random>]\n",
        argv[0]
    );
    return 1;
  ...
  io_params_t* thread_args = malloc(num_threads * sizeof(io_params_t));
